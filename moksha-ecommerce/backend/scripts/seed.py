...
class SeedProduct:
    ...
    stock: int
    description: str
    #: Extension of the artwork in `frontend/public/products/`. The generated range is SVG; the
    #: five Hydra Curls products are photographs shared with the Assignment 1 landing page.
    image_ext: str = "svg"

    @property
    def image_url(self) -> str:
        """Served by the frontend from `public/products/`.

        Most of the range is SVG generated by `frontend/scripts/generate-product-art.mjs`. The
        Hydra Curls line uses the real product photography from the Assignment 1 landing page —
        the same five bottles, so the two deliverables are visibly one brand rather than two
        unrelated demos.

        A root-relative path rather than an absolute URL, so the same seed data works on
        localhost, on a preview deployment and in production without a per-environment image
        host. Hotlinking a stock-photo CDN was the first approach and was abandoned: it
        rate-limits, it blocks hotlinking, and it made the demo's first paint depend on a third
        party.
        """
        return f"/products/{self.slug}.{self.image_ext}"
...
PRODUCTS: tuple[SeedProduct, ...] = (
    # --- The Hydra Curls line -------------------------------------------------------------
    #
    # These five are the products on the Assignment 1 landing page, with its photography. Same
    # names, same bottles: the shop sells the range the marketing page advertises, which is the
    ...
    SeedProduct(
        ...
        description=(
            ...
            "curls depend on. Hyaluronic acid and coconut-derived surfactants, 300 ml."
        ),
        image_ext="webp",
    ),
    SeedProduct(
        slug="hydra-curls-conditioner",
        name="Hydra Curls Hydrating Conditioner",
        category="condition",
        price_cents=54900,
        stock=96,
        description=(
    ...
    SeedProduct(
        slug="hydra-curls-defining-gel",
        name="Hydra Curls Defining Gel",
        category="style",
        price_cents=67900,
        stock=74,
        description=(
            "The 48-hour hold from the campaign, in a gel that scrunches out soft rather than "
            "crisp. Flaxseed, aloe and hyaluronic acid, alcohol-free, 250 ml."
        ),
        image_ext="webp",
    ),
    SeedProduct(
        slug="hydra-curls-defining-cream",
        name="Hydra Curls Defining Cream",
        category="style",
        price_cents=62900,
        stock=68,
        description=(
            "Definition without a cast, for curls that go limp under gel. Coconut and avocado "
            "oils seal the cuticle; layer it under the gel or wear it alone, 200 ml."
        ),
        image_ext="webp",
    ),
    SeedProduct(
        slug="hydra-curls-hydrating-mask",
        name="Hydra Curls Hydrating Mask",
        category="treatment",
        price_cents=94900,
        stock=47,
        description=(
            "Ten minutes a week to put back what heat and colour take out. The full hyaluronic "
            "acid concentration in the range, with murumuru butter, 200 ml."
        ),
        image_ext="webp",
    ),
    # --- The wider range ------------------------------------------------------------------
