...
async function api(path, token, { attempt = 1, soft = false } = {}) {
  ...
  if (res.status === 429) {
    if (attempt > 6) {
      ...
      fail('Figma rate limit exceeded. Wait a few minutes and re-run; already-downloaded files are skipped.');
    }
...
async function main() {
  ...
  if (!args.has('--skip-sections')) {
    ...
    const kids = frame.children
      ...
      .sort((a, b) => a.y - b.y)
      .map((k, i) => {
        const safe = k.name.replace(/[^a-z0-9]+/gi, '-').toLowerCase().slice(0, 40);
    ...
    log(`     ${kids.length - todo.length}/${kids.length} already present, ${todo.length} to render`);
